...
var Outside = {
	...
	setTitle: function() {
		...
		if(numHuts === 0) {
			...
			title = _("Một ngôi làng tổ chảng");
		} else if(numHuts <= 28) {
			title = _("Một ngôi làng to tổ chảng");
		} else if(numHuts <= 34) {
			title = _("Một thành phố mới nổi");
		} else if(numHuts <= 38) {
			title = _("Thành phố ngày càng đông đúc");
		} else if(numHuts <= 44) {
			title = _("Thành phố náo nhiệt");
		} else if(numHuts <= 48) {
			title = _("Thành phố lớn");
		} else {
			title = _("Thành phố vui vẻ");
		}
